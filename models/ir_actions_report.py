...
class IrActionsReport(models.Model):
    ...
    def _run_wkhtmltopdf(
        ...
        if self.paperformat_id and self.paperformat_id.format == 'A6':
            # get the number of pages
            pdf_reader = PdfFileReader(pdf_report_path)
            modified_pdf = PdfFileWriter()
            for i in range(pdf_reader.numPages):
                if i % 2 == 0: modified_pdf.addPage(pdf_reader.pages[i])
            with open(pdf_report_path, 'wb') as pdf_document:
                modified_pdf.write(pdf_document)
            with open(pdf_report_path, 'rb') as pdf_document:
                pdf_content = pdf_document.read()
